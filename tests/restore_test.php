...
final class restore_test extends \restore_date_testcase {
    /**
     * Test missing qtype_gapfill_options creation.
     *
     * @coversconvert_backup_to_questiondata() has #[\Override] attribute, but no matching parent method exists in ::restore()
     *
     * Old backup files may contain gapfill with no qtype_gapfill_options record.
    ...
    public function test_backup_and_restore(): void {
        ...
        $gapfill = $questiongenerator->create_question('gapfill', 'editor', ['category' => $category->id]);

        // Remove the options record, which means that the backup will look like a backup made in an old Moodle.
        // This line should include deletion of qtype_gapfill question id matching $gapfill->id but it causes an error.
        ...
        $newgapfill = $DB->get_record_sql('SELECT q.*
                                              FROM {question} q
                                              JOIN {question_versions} qv ON qv.questionid = q.id
                                              JOIN {question_bank_entries} qbe ON qbe.id = qv.questionbankentryid
                                             WHERE qbe.questioncategoryid = ?
                                               AND q.qtype = ?', [$newcategory->id, 'gapfill']);
        $this->assertTrue($DB->record_exists('question_gapfill', ['questionid' => $newgapfill->id]));
    }
}
